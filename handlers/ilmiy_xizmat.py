        ...
        # ── Maqola: bo'limma-bo'lim generatsiya ──────────────────────────────
            ...
            if service_key == "a_sci":
                plan_prompt = (
                    ...
                    "1. [Tadqiqotning asosiy natijalari]\n"
                    "2. [Qo'shimcha tahlil va isbotlar]\n"
            ...
            for line in plan_text.split("\n"):
                line = line.strip()
                m = re_mod.match(r'^[\*\-\#\s]*(\d+(?:\.\d+)?)\.?\s+[\*\*]*(.+)', line)
                if m:
                    plan_titles[m.group(1)] = m.group(2).replace('**', '').strip()
            ...
            for idx, (key, section_name, user_prompt_text) in enumerate(sections, 1):
                ...
                # Context injection to prevent hallucination across sections
                if plan_titles and key not in ["barcha_annotatsiyalar", "adabiyotlar"]:
                    plan_context = "DIQQAT! Hujjatning to'liq rejasi:\n" + "\n".join([f"{k}. {v}" for k, v in plan_titles.items()]) + "\n\nSiz ushbu rejadagi FAQAT '" + section_name + "' qismini yozishingiz kerak. Boshqa bo'limlarga o'tib ketmang."
                    contextual_prompt = plan_context + "\n\n" + user_prompt_text
                else:
                ...
                # Strip all markdown headings generated by AI to completely prevent duplicate/unwanted titles
                import re as re_mod_temp
                section_content = re_mod_temp.sub(r'(?m)^#+\s+.*$', '', section_content).strip()
                
                # Also strip if AI generated the title as plain ALL CAPS text (very common hallucination)
                sc_lines = section_content.split('\n')
                for _i, _l in enumerate(sc_lines):
                    _l_clean = _l.strip()
                    if _l_clean:
                        import difflib
